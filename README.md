...
[Диаграмма монолитного решения](/schemas/as-is.puml)
...
4. Поддомен "Инфраструктура". Сквозная функциональность и интеграционный конвейнера.
4.1 Контекст "Транзитных систем". Специально выделенная область, в которой размещаются внутренние интеграции во время переходного периода (разбиение монолита на микросервисы, интеграция с legacy-системами)
4.2 Контекст "Сквозная функциональность". Контекст, созданный для размещения сервисов, проходящих через разные слои и компоненты приложений.
...
Ссылка на [openapi.json](/apps/device-management/openapi.json) сервиса администрирования
Ссылка на [asyncapi.yml](/apps/legacy-monolith-temperature/asyncapi.yml) legacy-сервиса, метод создания нового устройства

Ниже приведен пример сценария по созданию автоматизированной цепочки действий сервиса администрирования
examples:
